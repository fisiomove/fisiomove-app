# (same as previous cell)  Fisiomove Pro 3.4.2 – Clinical Dark
# Rewriting full file to disk.
...
st.set_page_config(page_title="Fisiomove Pro 3.4.2 – Clinical Dark", layout="wide")
...
def radar_plot(labels, scores, title):
    ...
    if len(labels)==0:
        ...
        return fig
    labels_c=list(labels)+[labels[0]]; scores_c=list(scores)+[scores[0]]
...
def radar_png_buffer(labels, scores):
    ...
    angles=np.linspace(0,2*np.pi,N,endpoint=False).tolist(); angles_c=angles+[angles[0]]
    scores_c=list(scores)+[scores[0]]; ideal_c=[10]*N+[10]
...
def symmetry_bar_chart(sym_df):
    ...
    if sym_df is None or sym_df.empty:
        fig.update_layout(title="Nessun test bilaterale inserito", paper_bgcolor=BG, font=dict(color=TEXT), height=300); return fig
    fig.add_trace(go.Bar(x=sym_df["SymScore"], y=sym_df["Test"], orientation='h', marker=dict(color=[color_by_score(s) for s in sym_df["SymScore"]]), text=[f"{s:.1f}/10 (Delta {d})" for s,d in zip(sym_df["SymScore"], sym_df["DeltaStr"])], textposition="outside"))
...
def ebm_explain_row(test_name, unit, ref, val_avg, dx=None, sx=None):
    ...
    if "Lunge" in test_name:
        lines += ["- Dorsiflessione in carico: "+perf+".","- Valori ridotti spostano il carico sull'avampiede e anticipano la retroversione del bacino nello squat.","- Possibile aumento di stress femoro-rotuleo e perdita di stabilita' del tallone.","Riferimenti: Barton CJ 2012 JOSPT; Rabin A 2015 Phys Ther."]
    elif "Thoracic Extension" in test_name:
        lines += ["- Estensione toracica: "+perf+".","- Riduzioni favoriscono eccessiva inclinazione del tronco e compensi scapolo-omerali in squat/panca.","Riferimenti: Wilke J 2020 Front Physiol."]
    elif "AKE" in test_name or "SLR" in test_name:
        lines += ["- Estensibilita' catena posteriore: "+perf+".","- Limiti marcati riducono hinge efficiente e aumentano strain neurale/muscolare.","Riferimenti: Malliaras P 2015 Sports Med; Bohannon 1999 Phys Ther."]
    elif "ER" in test_name and "Shoulder" in test_name:
        lines += ["- Rotazione esterna spalla: "+perf+".","- Limitazioni in panca/low-bar aumentano stress anteriore e alterano il controllo scapolare.","Riferimenti: Ludewig 2009 Phys Ther; Green 2017 JSES."]
    elif "Pectoralis Minor" in test_name:
        lines += ["- Pettorale minore: "+perf+".","- Accorciamento associato a tilt anteriore scapola e riduzione spazio subacromiale.","Riferimenti: Borstad 2006 JSES."]
    elif "Schober" in test_name:
        lines += ["- Mobilita' lombare: "+perf+".","- Valori bassi riducono adattabilita' in flessione e spostano carico su anche/posteriore.","Riferimenti: Macrae 1990 Spine."]
    elif "Sorensen" in test_name:
        lines += ["- Endurance estensori lombari: "+perf+".","- Ridotta endurance = minore tolleranza a carichi prolungati in deadlift.","Riferimenti: Biering-Sorensen 1984 Spine; McGill 2001 Arch PMR."]
    elif "Thomas" in test_name:
        lines += ["- Estensione d'anca: "+perf+".","- Riduzioni aumentano tilt anteriore o iperlordosi in panca.","Riferimenti: Harvey 1998 PRI."]
    else:
        lines += ["- Prestazione "+perf+" rispetto al riferimento."]
...
st.markdown("<h2 style='text-align:center;margin-top:0'>Fisiomove Pro 3.4.2</h2>", unsafe_allow_html=True)
...
def page_valutazione():
    ...
    st.markdown("### Radar abilita' (media normalizzata)"); st.plotly_chart(radar_plot(df_meas.get("Test",pd.Series()).tolist(), pd.to_numeric(df_meas.get("Score",pd.Series()), errors="coerce").fillna(0).tolist(), f"{section} – Abilita' (0-10)"), use_container_width=True)
    st.markdown("### Simmetria Dx/Sx"); st.plotly_chart(symmetry_bar_chart(sym_df), use_container_width=True)
    ...
    with c1:
        if st.button("Salva"):
            save_session(st.session_state["athlete"], section, st.session_state["date"], df_show); st.success("Sessione salvata.")
    with c2:
        if st.button("JSON"):
...
def pdf_single_section(logo_bytes, athlete, evaluator, date_str, section_name, df, sym_df):
    ...
    story=[]; story.append(Paragraph("Fisiomove Pro 3.4.2 – Clinical Dark", title)); story.append(Spacer(1,6))
    ...
    if disp.empty:
        ...
        cols_order=["Test","Unità","Riferimento","Valore","Score","Dx","Sx","Delta","SymScore"]
        disp_pdf=_safe_pick(disp, cols_order)
        if "Unità" in disp_pdf.columns: disp_pdf.rename(columns={"Unità":"Unita'"}, inplace=True)
        if "Score" in disp_pdf.columns: disp_pdf["Score"]=pd.to_numeric(disp_pdf["Score"], errors="coerce").round(1)
        if "SymScore" in disp_pdf.columns: disp_pdf["SymScore"]=pd.to_numeric(disp_pdf["SymScore"], errors="coerce").round(1)
        table_data=[list(disp_pdf.columns)] + disp_pdf.fillna("").values.tolist()
        widths=[6.5*cm,1.5*cm,1.8*cm,1.8*cm,1.8*cm,1.5*cm,1.5*cm,1.8*cm,2.2*cm][:len(disp_pdf.columns)]
        ...
        rbuf=radar_png_buffer(disp.get("Test", pd.Series()).tolist(), pd.to_numeric(disp.get("Score", pd.Series()), errors="coerce").fillna(0).tolist())
        try:
            story.append(Image(io.BytesIO(rbuf.getvalue()), width=10.0*cm, height=10.0*cm)); story.append(Paragraph(f"Radar abilita' – {to_ascii(section_name)}", styles['Normal'])); story.append(Spacer(1,6))
        except Exception as e:
            story.append(Paragraph(f"Radar non disponibile ({to_ascii(str(e))}).", normal)); story.append(Spacer(1,6))
        sym_disp = sym_df.copy()
        if not sym_disp.empty:
            if "Unità" not in sym_disp.columns and "Unita'" in sym_disp.columns: sym_disp.rename(columns={"Unita'":"Unità"}, inplace=True)
            scols=["Test","Dx","Sx","Delta","SymScore"]; sym_disp=_safe_pick(sym_disp, scols)
            if not sym_disp.empty:
                if "SymScore" in sym_disp.columns: sym_disp["SymScore"]=pd.to_numeric(sym_disp["SymScore"], errors="coerce").round(1)
                if "Delta" in sym_disp.columns: sym_disp["Delta"]=pd.to_numeric(sym_disp["Delta"], errors="coerce").round(1)
                ...
                try: story.append(Image(io.BytesIO(sbuf.getvalue()), width=12.0*cm, height=8.0*cm)); story.append(Paragraph("Barre di simmetria Dx/Sx", styles['Normal']))
                except: pass
        story.append(Spacer(1,8)); story.append(Paragraph("Feedback clinico (EBM)", styles['Heading3']))
        for _, r in disp.iterrows():
            ...
            story.append(Paragraph(f"<b>{to_ascii(str(r.get('Test','')))}</b>", styles['Normal'])); story.append(flow); story.append(Spacer(1,4))
...
def page_riepilogo():
    ...
    dates=data_hist["date"].tolist(); pick=st.selectbox("Seleziona una valutazione salvata", dates, index=0)
    rec=data_hist[data_hist["date"]==pick].iloc[0]; df=pd.DataFrame(json.loads(rec["data_json"]))
    df_meas=df[df.get("Valore",0) > 0]
    st.markdown("### Radar abilita'")
    st.plotly_chart(radar_plot(df_meas.get("Test", pd.Series()).tolist(), pd.to_numeric(df_meas.get("Score", pd.Series()), errors="coerce").fillna(0).tolist(), f"{section} – Abilita' (0-10) @ {pick}"), use_container_width=True)
    ...
    st.markdown("### Barre di simmetria"); st.plotly_chart(symmetry_bar_chart(sym_df), use_container_width=True)
    st.markdown("### Tabella EBM")
    show_cols=[c for c in ["Test","Unità","Unita'","Valore","Score","Dx","Sx","Delta","SymScore","CommentoEBM"] if c in df.columns]
    df_show=df[show_cols].copy()
    if "Unità" not in df_show.columns and "Unita'" in df_show.columns: df_show.rename(columns={"Unita'":"Unità"}, inplace=True)
    if "Unità" in df_show.columns: df_show["Unità"]=df_show["Unità"].apply(to_ascii)
    if "CommentoEBM" in df_show.columns: df_show["CommentoEBM"]=df_show["CommentoEBM"].apply(to_ascii)
    st.dataframe(df_show.style.format({"Score":"{:.1f}","SymScore":"{:.1f}","Valore":"{:.1f}","Dx":"{:.1f}","Sx":"{:.1f}","Delta":"{:.1f}"}), use_container_width=True)
...
def page_storico():
    ...
    comp=[]
    for d in pick[:3]:
        rec=data_hist[data_hist["date"]==d].iloc[0]; df_rec=pd.DataFrame(json.loads(rec["data_json"])); df_rec=df_rec[df_rec.get("Valore",0) > 0]; comp.append((d,df_rec))
    if not comp: st.warning("Seleziona almeno una data."); return
    labels=sorted({t for _,dfc in comp for t in dfc.get("Test", pd.Series()).tolist()}); fig=go.Figure()
    if labels:
    ...
    for d, dfc in comp:
        scores_map={t:s for t,s in zip(dfc.get("Test", pd.Series()), pd.to_numeric(dfc.get("Score", pd.Series()), errors="coerce").fillna(0))}; series=[scores_map.get(t,0) for t in labels]; series_c=series+([series[0]] if labels else [])
...
st.session_state.setdefault("page","Valutazione")
st.session_state["section"]=st.selectbox("Movimento testato", options=list(TESTS.keys()), index=list(TESTS.keys()).index(st.session_state.get("section","Squat")))
render_page()
