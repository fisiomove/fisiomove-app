...
def pdf_single_section(logo_bytes, athlete, evaluator, date_str, section_name, df):
    ...
    if disp.empty:
        ...
        if img and len(img) > 100:
            story.append(Image(ImageReader(io.BytesIO(img)), width=10*cm, height=10*cm)); story.append(Spacer(1, 6))
        else:
            story.append(Paragraph("Nessun grafico radar disponibile (nessun test valido o errore grafico).", normal)); story.append(Spacer(1, 6))
