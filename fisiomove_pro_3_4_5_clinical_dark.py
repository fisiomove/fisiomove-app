# Fisiomove Pro 3.4.5 – Clinical Dark
# - Valutazione Generale (tutti i test in un unico radar)
# - Simmetria angolare 20°, cm 8, % 20
# - Tutorial EBM (expander)
# - PDF: header con logo su OGNI pagina (onFirstPage/onLaterPages)
# - Tema dark mobile friendly, layout pulito
# - NEW: Autosave/Autoload ultimo test per sezione + Reset sezione
# - NEW: Inizializzazione casuale realistica se non esistono dati salvati

import io, os, json, sqlite3, datetime as dt, random
import numpy as np
import pandas as pd
...
st.set_page_config(page_title="Fisiomove Pro 3.4.5 – Clinical Dark", layout="wide")
...
st.markdown("<h2 style='text-align:center;margin-top:0'>Fisiomove Pro 3.4.5</h2>", unsafe_allow_html=True)
...
def rand_val(ref, unit):
    """Random realistico: ±15% ref; bilaterale differenza 3–10 gradi/cm in media"""
    if unit == "sec":
...
def load_or_seed_values(section):
    ...
    if section == "Valutazione Generale":
        # non ha senso caricare random; i valori vengono da sezioni
        # costruiamo un dizionario aggregando ultimi valori per ogni sezione
        agg = {}
        for sec in ALL_ORDER:
            ...
            for _, (name, unit, ref, desc, bilateral) in iterate_tests_for_section(sec):
                if name in saved:
                    ...
                    # se proprio non esiste, seed random
                    if bilateral:
...
def page_valutazione():
    ...
    for sec_name, (name, unit, ref, desc, bilateral) in iterate_tests_for_section(section):
        ...
        if bilateral:
            ...
            with c1:
                ...
                dx = st.slider(f"Dx ({unit}, rif. {ref})", 0.0, float(maxv), float(dx_val), 0.1, key=f"{section}_{name}_dx")
            with c2:
                sx_val = per_test_values[name]["Sx"]
                sx = st.slider(f"Sx ({unit}, rif. {ref})", 0.0, float(maxv), float(sx_val), 0.1, key=f"{section}_{name}_sx")
            # update in state
            ...
            val = st.slider(f"{unit} (rif. {ref})", 0.0, float(maxv), float(val0), 0.1, key=f"{section}_{name}")
    ...
    with c4:
        if st.button("Reset sezione"):
            # cancella seed e valori della sezione corrente
