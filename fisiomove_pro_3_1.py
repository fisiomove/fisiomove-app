# Fisiomove Pro 3.1 – Streamlit app (Final)
...
# - Matplotlib backend "Agg" for Streamlit Cloud
# - Radar style toggle (Minimal vs Avanzato)
...
PRIMARY = "#004A9F"
DEFAULT_LOGO_PATH = "logo_fisiomove.png"  # optional local logo file
DB_PATH = "fisiomove.db"

# ---------- PAGE CONFIG ----------
st.set_page_config(page_title="Fisiomove Pro 3.1 – Mobilità EBM", layout="wide")
...
def radar_plot(labels, scores, title, advanced=True):
    # Plotly radar for on-screen visualization
    if len(labels) == 0:
        fig = go.Figure()
        fig.update_layout(
    ...
    radial = dict(visible=True, range=[0,10])
    if advanced:
        radial.update(dict(tickmode="linear", dtick=1, gridcolor="#cccccc"))
    fig.update_layout(
        title=title,
        showlegend=True,
...
def radar_png_buffer(labels, scores, advanced=True):
    """Return a PNG buffer for the PDF; robust in cloud & supports 1+ points."""
    N = len(labels)
    buf = io.BytesIO()
    if N == 0:
        return buf

    # For 1 point, duplicate to make a minimal shape; for 2+ regular polygon
    if N == 1:
        labels = labels + labels  # duplicate
    ...
    # Aesthetic radar for PDF
    fig, ax = plt.subplots(figsize=(4.8, 4.8), subplot_kw=dict(polar=True))
    # Grid / radial styling
    if advanced:
        ax.set_facecolor("white")
        ax.grid(True, linewidth=0.6, color="#cccccc", alpha=0.9)
    ...
    # Athlete polygon
    ax.plot(angles_c, scores_c, color="#1f6fe0", linewidth=2, label="Atleta")
    ax.fill(angles_c, scores_c, color="#1f6fe033")
    ...
    if advanced:
        # Show radial value labels near vertices
        for angle, r in zip(angles, scores):
            ax.text(angle, r + 0.4, f"{r:.1f}", ha="center", va="center", fontsize=7, color="#003366")
...
def pdf_single_section(logo_bytes, athlete, evaluator, date_str, section_name, df, advanced=True):
    ...
    story = []
    story.append(Paragraph("Fisiomove Pro 3.1 – Mobilità EBM", title))
    ...
    if disp.empty:
        ...
        # Radar image (robust)
        rbuf = radar_png_buffer(disp["Test"].tolist(), disp["Score"].tolist(), advanced=advanced)
        try:
            img = rbuf.getvalue()
            if img and len(img) > 100:
                story.append(Image(ImageReader(io.BytesIO(img)), width=11*cm, height=11*cm))
...
with col_title:
    st.markdown("<h1 style='margin-bottom:0'>Fisiomove Pro 3.1</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    radar_style_adv = st.toggle("🧭 Stile radar avanzato", value=True)
...
with tabs[0]:
    ...
    st.markdown("### Radar")
    fig = radar_plot(df_meas["Test"].tolist(), df_meas["Score"].tolist(), f"{section} – Punteggi (0–10)", advanced=radar_style_adv)
    ...
    with c3:
        ...
        if st.button("Genera PDF", disabled=disabled_pdf):
            # Try to load local logo file; optional
            try:
                with open(DEFAULT_LOGO_PATH, "rb") as f:
                    logo_bytes = f.read()
            except Exception:
                logo_bytes = None
            pdf = pdf_single_section(logo_bytes, athlete, evaluator, date_str, section, df, advanced=radar_style_adv)
